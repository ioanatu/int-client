...
React client for the [IntNext PoC API](../int-server) — browse supplier master data with
search, filtering, pagination and a full supplier profile view.
...
For a build that has to reach the backend directly, `VITE_API_BASE_URL` and
...
| `VITE_API_BASE_URL`  | browser            | Bypass the proxy and call the API directly       |
...
- **Errors** — `src/api/errors.ts` unwraps the backend's error envelope, so the user sees
  *“Supplier with id 'sup_999' was not found.”* rather than a status code. Transport
